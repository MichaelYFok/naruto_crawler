...
class ComicSpider(scrapy.Spider):
    ...
    def __init__(self):
        # 图片链接server域名
        self.sever_img = 'http://n.1whour.com'
        # 章节链接server域名
    ...
    def parse1(self, response):
        ...
        # 章节名
        dir_name = hxs.xpath('//dd/a[1]/text()').extract()
        # 保存章节链接和章节名
    ...
    def parse2(self, response):
        ...
        # 返回的图片地址，应该为列表，否则会报错
        img_url = [self.sever_img + re.findall(self.pattern_img, pre_img_url[0])[0]]
        # 将获取的章节的第一页图片链接保存到img_url中
        item['img_url'] = img_url
        # 返回item，交给item pipeline下载图片
        yield item
        # 获取章节的页数
        page_num = hxs.xpath('//td[@valign="top"]/text()').re('共(\d+)页')[0]
        # 根据页数，整理出本章节其他页码的链接
